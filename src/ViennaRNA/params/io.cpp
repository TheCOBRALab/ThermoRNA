...
char* last_param_file = NULL;

int stack_dim[2] = {NBPAIRS + 1, NBPAIRS + 1};
int stack_shift[2] = {1, 1};
int mismatch_dim[3] = {NBPAIRS + 1, 5, 5};
int mismatch_shift[3] = {1, 0, 0};
int int11_dim[4] = {NBPAIRS + 1, NBPAIRS + 1, 5, 5};
int int11_shift[4] = {1, 1, 0, 0};
int int21_dim[5] = {NBPAIRS + 1, NBPAIRS + 1, 5, 5, 5};
int int21_shift[5] = {1, 1, 0, 0, 0};
int int22_dim[6] = {NBPAIRS + 1, NBPAIRS + 1, 5, 5, 5, 5};
int int22_shift[6] = {1, 1, 1, 1, 1, 1};
int int22_post[6] = {1, 1, 0, 0, 0, 0};
int dangle_dim[2] = {NBPAIRS + 1, 5};
int dangle_shift[2] = {1, 0};
...
void display_array(int* p, int size, int line, FILE* fp);

char* get_array1(char** content, size_t* line_no, int* arr, int size);

bool ignore_comment(char* line);

void check_symmetry(void);

void update_nst(int array[NBPAIRS + 1][NBPAIRS + 1][5][5][5][5]);
...
**/
bool rd_1dim(char** content, size_t* line_no, int* array, int dim, int shift);

bool rd_1dim_slice(char** content, size_t* line_no, int* array, int dim, int shift, int post);

bool rd_2dim(char** content, size_t* line_no, int* array, int dim[2], int shift[2]);

bool rd_2dim_slice(char** content, size_t* line_no, int* array, int dim[2], int shift[2],
                   int post[2]);

bool rd_3dim(char** content, size_t* line_no, int* array, int dim[3], int shift[3]);

bool rd_3dim_slice(char** content, size_t* line_no, int* array, int dim[3], int shift[3],
                   int post[3]);

bool rd_4dim(char** content, size_t* line_no, int* array, int dim[4], int shift[4]);

bool rd_4dim_slice(char** content, size_t* line_no, int* array, int dim[4], int shift[4],
                   int post[4]);

bool rd_5dim(char** content, size_t* line_no, int* array, int dim[5], int shift[5]);

bool rd_5dim_slice(char** content, size_t* line_no, int* array, int dim[5], int shift[5],
                   int post[5]);

[[maybe_unused]]
bool rd_6dim(char** content, size_t* line_no, int* array, int dim[6], int shift[6]);

bool rd_6dim_slice(char** content, size_t* line_no, int* array, int dim[6], int shift[6],
                   int post[6]);

void rd_Tetraloop37(char** content, size_t* line_no);

void rd_Triloop37(char** content, size_t* line_no);

void rd_Hexaloop37(char** content, size_t* line_no);

int set_parameters_from_string(char** file_content, const char* name);

char** file2array(const char fname[]);

int save_parameter_file(const char fname[], unsigned int options);
...
/*
 ...
 #################################
 */
PUBLIC int vrna_params_load(const char fname[], [[maybe_unused]] unsigned int options) {
    ...
    if (file_content) {
        ...
        ret = set_parameters_from_string(file_content, (const char*) name);

        free(name);
        for (ptr = file_content; *ptr != NULL; ptr++) free(*ptr);
...
PUBLIC int vrna_params_save(const char fname[], unsigned int options) {
    return save_parameter_file(fname, options);
}

PUBLIC int vrna_params_load_from_string(const char* string, const char* name,
    ...
    if (string) {
        ...
        ret = set_parameters_from_string(params_array, name);

        /* cleanup memory */
        free(tmp_string);

        for (ptr = params_array; *ptr != NULL; ptr++) free(*ptr);
 ...
 *###########################################
 */
PUBLIC const char* last_parameter_file(void) { return (const char*) last_param_file; }

/*------------------------------------------------------------*/
PUBLIC void read_parameter_file(const char fname[]) {
    int ret = vrna_params_load(fname, VRNA_PARAMETER_FORMAT_DEFAULT);
    if (!ret) vrna_log_warning("Failed to load parameters from file \"%s\"", fname);
}

PUBLIC const char* settype(enum parset s) {
    switch (s) {
        case S:       return "stack";
        ...
        case IL:      return "internal";
        case IL_H:    return "internal_enthalpies";
        ...
        case MMI:     return "mismatch_internal";
        case MMI_H:   return "mismatch_internal_enthalpies";
        case MMI1N:   return "mismatch_internal_1n";
        case MMI1N_H: return "mismatch_internal_1n_enthalpies";
        case MMI23:   return "mismatch_internal_23";
        case MMI23_H: return "mismatch_internal_23_enthalpies";
...
PUBLIC enum parset gettype(const char* ident) {
    if (strcmp(ident, "stack") == 0) {
        ...
        vrna_log_warning(
            "Detected  deprecated identifier 'interior_enthalpies'! Use 'internal_enthalpies' "
            "instead!");
        return IL_H;
        ...
        vrna_log_warning(
            "Detected  deprecated identifier 'mismatch_interior'! Use 'mismatch_internal' "
            "instead!");
        return MMI;
    } else if (strcmp(ident, "mismatch_interior_enthalpies") == 0) {
        vrna_log_warning(
            "Detected  deprecated identifier 'mismatch_interior_enthalpies'! Use "
            "'mismatch_internal_enthalpies' instead!");
        return MMI_H;
    } else if (strcmp(ident, "mismatch_interior_1n") == 0) {
        vrna_log_warning(
            "Detected  deprecated identifier 'mismatch_interior_1n'! Use 'mismatch_internal_1n' "
            "instead!");
        return MMI1N;
    } else if (strcmp(ident, "mismatch_interior_1n_enthalpies") == 0) {
        vrna_log_warning(
            "Detected  deprecated identifier 'mismatch_interior_1n_enthalpies'! Use "
            "'mismatch_internal_1n_enthalpies' instead!");
        return MMI1N_H;
    } else if (strcmp(ident, "mismatch_interior_23") == 0) {
        vrna_log_warning(
            "Detected  deprecated identifier 'mismatch_interior_23'! Use 'mismatch_internal_23' "
            "instead!");
        return MMI23;
    } else if (strcmp(ident, "mismatch_interior_23_enthalpies") == 0) {
        vrna_log_warning(
            "Detected  deprecated identifier 'mismatch_interior_23_enthalpies'! Use "
            "'mismatch_internal_23_enthalpies' instead!");
        return MMI23_H;
...
PUBLIC void write_parameter_file(const char fname[]) {
    FILE* outfp;
    size_t c;
    const char* pnames[] = {"NP", "CG", "GC", "GU", "UG", "AU", "UA", " @"};
    ...
    for (c = 1; c < NBPAIRS + 1; c++) display_array(stack37[c] + 1, NBPAIRS, NBPAIRS, outfp);

    fprintf(outfp, "\n# %s\n", settype(S_H));
    fprintf(outfp, "/*  CG    GC    GU    UG    AU    UA    @  */\n");
    for (c = 1; c < NBPAIRS + 1; c++) display_array(stackdH[c] + 1, NBPAIRS, NBPAIRS, outfp);

    fprintf(outfp, "\n# %s\n", settype(MMH));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchH37[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMH_H));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchHdH[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMI));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchI37[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMI_H));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchIdH[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMI1N));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatch1nI37[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMI1N_H));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatch1nIdH[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMI23));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatch23I37[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMI23_H));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatch23IdH[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMM));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchM37[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MMM_H));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchMdH[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MME));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchExt37[k][i], 5, 5, outfp);
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(MME_H));
    {
        size_t i, k;
        for (k = 1; k < NBPAIRS + 1; k++)
            for (i = 0; i < 5; i++) display_array(mismatchExtdH[k][i], 5, 5, outfp);
    }

    fprintf(outfp, "\n# %s\n", settype(D5));
    fprintf(outfp, "/*  @     A     C     G     U   */\n");
    for (c = 1; c < NBPAIRS + 1; c++) display_array(dangle5_37[c], 5, 5, outfp);

    fprintf(outfp, "\n# %s\n", settype(D5_H));
    fprintf(outfp, "/*  @     A     C     G     U   */\n");
    for (c = 1; c < NBPAIRS + 1; c++) display_array(dangle5_dH[c], 5, 5, outfp);

    fprintf(outfp, "\n# %s\n", settype(D3));
    fprintf(outfp, "/*  @     A     C     G     U   */\n");
    for (c = 1; c < NBPAIRS + 1; c++) display_array(dangle3_37[c], 5, 5, outfp);

    fprintf(outfp, "\n# %s\n", settype(D3_H));
    fprintf(outfp, "/*  @     A     C     G     U   */\n");
    for (c = 1; c < NBPAIRS + 1; c++) display_array(dangle3_dH[c], 5, 5, outfp);
    ...
    fprintf(outfp, "\n# %s\n", settype(INT11));
    {
        ...
        for (k = 1; k < NBPAIRS + 1; k++)
            for (l = 1; l < NBPAIRS + 1; l++) {
                fprintf(outfp, "/* %2s..%2s */\n", pnames[k], pnames[l]);
                for (i = 0; i < 5; i++) display_array(int11_37[k][l][i], 5, 5, outfp);
            }
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(INT11_H));
    {
        ...
        for (k = 1; k < NBPAIRS + 1; k++)
            for (l = 1; l < NBPAIRS + 1; l++) {
                fprintf(outfp, "/* %2s..%2s */\n", pnames[k], pnames[l]);
                for (i = 0; i < 5; i++) display_array(int11_dH[k][l][i], 5, 5, outfp);
            }
    }
        ...
        for (p1 = 1; p1 < NBPAIRS + 1; p1++)
            for (p2 = 1; p2 < NBPAIRS + 1; p2++)
                for (i = 0; i < 5; i++) {
                    ...
                    for (j = 0; j < 5; j++) display_array(int21_37[p1][p2][i][j], 5, 5, outfp);
                }
    }
    ...
    fprintf(outfp, "\n# %s\n", settype(INT21_H));
    {
        ...
        for (p1 = 1; p1 < NBPAIRS + 1; p1++)
            for (p2 = 1; p2 < NBPAIRS + 1; p2++)
                for (i = 0; i < 5; i++) {
                    fprintf(outfp, "/* %2s.%c..%2s */\n", pnames[p1], bnames[i], pnames[p2]);
                    for (j = 0; j < 5; j++) display_array(int21_dH[p1][p2][i][j], 5, 5, outfp);
                }
    }
    ...
    fprintf(outfp, "\t%6d\t%6d\t%6d\t%6d\t%6d\t%6d\n", ML_BASE37, ML_BASEdH, ML_closing37,
            ML_closingdH, ML_intern37, ML_interndH);
    ...
    fprintf(outfp, "   %6d %6d %6d  %6d %3.6f %6d\n", DuplexInit37, DuplexInitdH, TerminalAU37,
            TerminalAUdH, lxc37, 0);
        ...
        if (r == 1) {
            ...
            switch (type) {
                ...
                case S_H:
                    ...
                    break;
                case HP: success &= rd_1dim(file_content, &line_no, &(hairpin37[0]), 31, 0); break;
                case HP_H:
                    success &= rd_1dim(file_content, &line_no, &(hairpindH[0]), 31, 0);
                    break;
                case B:   success &= rd_1dim(file_content, &line_no, &(bulge37[0]), 31, 0); break;
                case B_H: success &= rd_1dim(file_content, &line_no, &(bulgedH[0]), 31, 0); break;
                case IL:
                ...
                case MMM_H:
                    ...
                    break;
                case INT11:
                    success &= rd_4dim(file_content, &line_no, &(int11_37[0][0][0][0]), int11_dim,
                                       int11_shift);
                    break;
                case INT11_H:
                    success &= rd_4dim(file_content, &line_no, &(int11_dH[0][0][0][0]), int11_dim,
                                       int11_shift);
                    break;
                case INT21:
                    success &= rd_5dim(file_content, &line_no, &(int21_37[0][0][0][0][0]),
                                       int21_dim, int21_shift);
                    break;
                case INT21_H:
                ...
                case INT22: {
                    bool ok = rd_6dim_slice(file_content, &line_no, &(int22_37[0][0][0][0][0][0]),
                                            int22_dim, int22_shift, int22_post);
                    success &= ok;
                ...
                case D5:
                    success &= rd_2dim(file_content, &line_no, &(dangle5_37[0][0]), dangle_dim,
                                       dangle_shift);
                    break;
                case D5_H:
                    success &= rd_2dim(file_content, &line_no, &(dangle5_dH[0][0]), dangle_dim,
                                       dangle_shift);
                    break;
                case D3:
                    success &= rd_2dim(file_content, &line_no, &(dangle3_37[0][0]), dangle_dim,
                                       dangle_shift);
                    break;
                case D3_H:
                    success &= rd_2dim(file_content, &line_no, &(dangle3_dH[0][0]), dangle_dim,
                                       dangle_shift);
                    break;
                case ML: {
                    int values[6];
                ...
                case TL:  rd_Tetraloop37(file_content, &line_no); break;
                case TRI: rd_Triloop37(file_content, &line_no); break;
                case HEX: rd_Hexaloop37(file_content, &line_no); break;
...
char* get_array1(char** content, size_t* line_no, int* arr, int size) {
    ...
    while (i < size) {
        ...
        if (!ignore_comment(line)) return line;
...
bool rd_3dim(char** content, size_t* line_no, int* array, int dim[3], int shift[3]) {
    ...
    return rd_3dim_slice(content, line_no, array, dim, shift, post);
}
...
bool rd_3dim_slice(char** content, size_t* line_no, int* array, int dim[3], int shift[3],
    ...
    for (i = shift[0]; i < dim[0] - post[0]; i++) {
        if (!rd_2dim_slice(content, line_no, array + (i * dim[1] * dim[2]), dim + 1, shift + 1,
                           post + 1)) {
            return false;
        }
    }
...
bool rd_4dim(char** content, size_t* line_no, int* array, int dim[4], int shift[4]) {
    ...
    return rd_4dim_slice(content, line_no, array, dim, shift, post);
}
...
bool rd_4dim_slice(char** content, size_t* line_no, int* array, int dim[4], int shift[4],
    ...
    for (i = shift[0]; i < dim[0] - post[0]; i++) {
        if (!rd_3dim_slice(content, line_no, array + (i * dim[1] * dim[2] * dim[3]), dim + 1,
                           shift + 1, post + 1)) {
            return false;
        }
    }
...
bool rd_5dim(char** content, size_t* line_no, int* array, int dim[5], int shift[5]) {
    ...
    return rd_5dim_slice(content, line_no, array, dim, shift, post);
}
...
bool rd_6dim(char** content, size_t* line_no, int* array, int dim[6], int shift[6]) {
    ...
    return rd_6dim_slice(content, line_no, array, dim, shift, post);
}
...
bool rd_6dim_slice(char** content, size_t* line_no, int* array, int dim[6], int shift[6],
    ...
    if (delta_pre + delta_post == 0) {
        return rd_1dim(content, line_no, array, dim[0] * dim[1] * dim[2] * dim[3] * dim[4] * dim[5],
                       0);
    }

    for (i = shift[0]; i < dim[0] - post[0]; i++)
        if (!rd_5dim_slice(content, line_no,
                           array + (i * dim[1] * dim[2] * dim[3] * dim[4] * dim[5]), dim + 1,
                           shift + 1, post + 1)) {
            return false;
        }
    return true;
...
bool ignore_comment(char* line) {
    ...
    if ((cp1 = strstr(line, "/*"))) {
        ...
        for (cp2 += 2; *cp2 != '\0'; cp2++, cp1++) *cp1 = *cp2;
        *cp1 = '\0';
    }
...
void check_symmetry(void) {
    ...
    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j <= NBPAIRS; j++)
            if (stack37[i][j] != stack37[j][i]) vrna_log_warning("stacking energies not symmetric");
                    ...
                    for (m = 0; m < 5; m++)
                        for (n = 0; n < 5; n++)
                            if (int22_dH[i][j][k][l][m][n] != int22_dH[j][i][m][n][k][l])
                                vrna_log_warning(
                                    "int22 enthalpies not symmetric: %d %d %d %d %d %d", i, j, k, l,
                                    m, n);
                }
}
...
void update_nst(int array[NBPAIRS + 1][NBPAIRS + 1][5][5][5][5]) {
            ...
            for (k = 1; k < 5; k++) {
                for (l = 1; l < 5; l++) {
                    for (m = 1; m < 5; m++) {
                        ...
                        for (n = 1; n < 5; n++) {
                            max = std::max(max, array[i][j][k][l][m][n]);
                            max2 = std::max(max2, array[i][j][k][l][n][m]);
                            max3 = std::max(max3, array[i][j][k][n][l][m]);
                            max4 = std::max(max4, array[i][j][n][k][l][m]);
                        }
        ...
        for (j = 1; j < NBPAIRS; j++) {
            for (k = 1; k < 5; k++) {
                for (l = 1; l < 5; l++) {
                    ...
                    for (m = 1; m < 5; m++) {
                        max = std::max(max, array[i][j][k][l][m][0]);
                        max2 = std::max(max2, array[i][j][k][m][0][l]);
                        max3 = std::max(max3, array[i][j][m][0][k][l]);
                        max4 = std::max(max4, array[i][j][0][k][l][m]);
                        max5 = std::max(max5, array[i][j][0][k][m][l]);
                        max6 = std::max(max6, array[i][j][k][0][l][m]);
                    }
        ...
        for (j = 1; j < NBPAIRS; j++) {
            for (k = 1; k < 5; k++) {
                ...
                for (l = 1; l < 5; l++) {
                    ...
                    max = std::max(max, array[i][j][k][l][0][0]);
                    max2 = std::max(max2, array[i][j][0][k][l][0]);
                    max3 = std::max(max3, array[i][j][0][0][k][l]);
                    max4 = std::max(max4, array[i][j][0][0][l][k]);
                }
    ...
    for (i = 1; i < NBPAIRS; i++) {
        for (j = 1; j < NBPAIRS; j++) {
            max = -INF; /* max of {CGAU} */
            for (k = 1; k < 5; k++) max = std::max(max, array[i][j][k][0][0][0]);
            ...
            for (l = 0; l < 5; l++) {
                for (m = 0; m < 5; m++) {
                    for (n = 0; n < 5; n++) {
                        ...
                        for (j = 1; j < NBPAIRS; j++) {
                            max = std::max(max, array[i][j][k][l][m][n]);
                            max2 = std::max(max2, array[j][i][k][l][m][n]);
                        }
        ...
        for (l = 0; l < 5; l++) {
            for (m = 0; m < 5; m++) {
                for (n = 0; n < 5; n++) {
                    max = -INF;
                    for (j = 1; j < NBPAIRS; j++)
                        max = std::max(max, array[NBPAIRS][j][k][l][m][n]);
...
char* vrna_basename(const char* path) {
    ...
    if (path) {
        const char* ptr = strrchr(path, DIRSEPC);
...
char* vrna_read_line(FILE* fp) {
    ...
    do {
        if (fgets(s, sizeof(s), fp) == nullptr) break;

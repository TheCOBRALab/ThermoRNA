...
/* Rescale Free energy contribution according to deviation of temperature from measurement
 * conditions */
#define RESCALE_dG(dG, dH, dT) static_cast<int>((dH) - ((dH) - (dG)) * (dT))

/*
 * Rescale Free energy contribution according to deviation of temperature from measurement
 * conditions and convert it to Boltzmann Factor for specific kT
 */
#define RESCALE_BF(dG, dH, dT, kT) \
    (exp(-TRUNC_MAYBE((double) RESCALE_dG((dG), (dH), (dT))) * 10. / kT))

#define RESCALE_BF_SMOOTH(dG, dH, dT, kT) \
    (exp(SMOOTH(-TRUNC_MAYBE((double) RESCALE_dG((dG), (dH), (dT)))) * 10. / kT))
...
PRIVATE vrna_param_t p_pre{};
PRIVATE unsigned char p_pre_init = 0;

PRIVATE vrna_exp_param_t pf_pre{};
...
/*
 ...
 #################################
 */
PUBLIC vrna_param_t* vrna_params(vrna_md_t* md_p) {
...
PUBLIC vrna_exp_param_t* vrna_exp_params(vrna_md_t* md_p) {
...
PUBLIC vrna_exp_param_t* vrna_exp_params_comparative(unsigned int n_seq, vrna_md_t* md) {
    if (md) {
        return get_exp_params_ali(md, n_seq, -1.);
    } else {
        vrna_md_t md_;
        vrna_md_set_default(&md_);
        return get_exp_params_ali(&md_, n_seq, -1.);
    }
}

PUBLIC vrna_param_t* vrna_params_copy(vrna_param_t* par) {
...
PUBLIC vrna_exp_param_t* vrna_exp_params_copy(vrna_exp_param_t* par) {
...
PRIVATE vrna_param_t* get_scaled_params(vrna_md_t* md) {
    ...
    double tempf;
    vrna_param_t* params;
    double salt_conc = md->salt;
    double salt_standard  = VRNA_MODEL_DEFAULT_SALT;

    params = (vrna_param_t*) vrna_alloc(sizeof(vrna_param_t));

    memset(params->param_file, '\0', 256);
    if (last_parameter_file() != NULL) strncpy(params->param_file, last_parameter_file(), 255);
    ...
    for (i = 0; i < 31; i++) params->hairpin[i] = RESCALE_dG(hairpin37[i], hairpindH[i], tempf);
    ...
    for (; i <= MAXLOOP; i++) {
        ...
        params->SaltLoopDbl[i] =
            (salt_conc == salt_standard) ? 0. : vrna_salt_loop(i, salt_conc, salt_standard, md->backbone_length);
        ...
        for (j = 0; j < 5; j++)
            for (k = 0; k < 5; k++) {
                ...
                params->mismatchI[i][j][k] =
                    RESCALE_dG(mismatchI37[i][j][k], mismatchIdH[i][j][k], tempf);
                params->mismatchH[i][j][k] =
                    RESCALE_dG(mismatchH37[i][j][k], mismatchHdH[i][j][k], tempf);
                params->mismatch1nI[i][j][k] =
                    RESCALE_dG(mismatch1nI37[i][j][k], mismatch1nIdH[i][j][k], tempf);
                params->mismatch23I[i][j][k] =
                    RESCALE_dG(mismatch23I37[i][j][k], mismatch23IdH[i][j][k], tempf);
                if (md->dangles) {
            ...
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++)
                    params->int11[i][j][k][l] =
                        RESCALE_dG(int11_37[i][j][k][l], int11_dH[i][j][k][l], tempf);
                ...
                for (l = 0; l < 5; l++) {
                    ...
                    for (m = 0; m < 5; m++)
                        params->int21[i][j][k][l][m] =
                            RESCALE_dG(int21_37[i][j][k][l][m], int21_dH[i][j][k][l][m], tempf);
                }
        ...
        for (j = 0; j <= NBPAIRS; j++)
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++) {
                    ...
                    for (m = 0; m < 5; m++)
                        for (n = 0; n < 5; n++)
                            params->int22[i][j][k][l][m][n] = RESCALE_dG(
                                int22_37[i][j][k][l][m][n], int22_dH[i][j][k][l][m][n], tempf);
                }
    ...
    params->SaltStack = (salt_conc == salt_standard) ? 0 : vrna_salt_stack(salt_conc, salt_standard, md->helical_rise);
    if (salt_conc == salt_standard)
        params->SaltMLbase = params->SaltMLclosing = 0;
    else
        vrna_salt_ml(params->SaltLoopDbl, md->saltMLLower, md->saltMLUpper, &params->SaltMLbase,
                     &params->SaltMLclosing);

    params->MLclosing += params->SaltMLbase;
    params->MLclosing += params->SaltMLclosing;
    params->MLbase += params->SaltMLbase;
    for (i = 0; i <= NBPAIRS; i++) params->MLintern[i] += params->SaltMLbase;
...
PRIVATE vrna_exp_param_t* get_scaled_exp_params(vrna_md_t* md, double pfs) {
    ...
    double kT, TT;
    double GT;
    double salt_conc, salt_standard;
    vrna_exp_param_t* pf;

    pf = (vrna_exp_param_t*) vrna_alloc(sizeof(vrna_exp_param_t));

    memset(pf->param_file, '\0', 256);
    if (last_parameter_file() != NULL) strncpy(pf->param_file, last_parameter_file(), 255);
    ...
    salt_conc = md->salt;
    salt_standard = VRNA_MODEL_DEFAULT_SALT;
    ...
    for (i = 0; i < 31; i++) pf->exphairpin[i] = RESCALE_BF(hairpin37[i], hairpindH[i], TT, kT);
    ...
    if (salt_conc == salt_standard) {
        ...
        for (i = 0; i <= MIN2(30, MAXLOOP); i++) {
            pf->SaltLoopDbl[i] = vrna_salt_loop(i, salt_conc, salt_standard, md->backbone_length);
    ...
    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j <= 4; j++) {
            if (md->dangles) {
                pf->expdangle5[i][j] =
                    RESCALE_BF_SMOOTH(dangle5_37[i][j], dangle5_dH[i][j], TT, kT);
                pf->expdangle3[i][j] =
                    RESCALE_BF_SMOOTH(dangle3_37[i][j], dangle3_dH[i][j], TT, kT);
            } else {
    ...
    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j < 5; j++)
            for (k = 0; k < 5; k++) {
                pf->expmismatchI[i][j][k] =
                    RESCALE_BF(mismatchI37[i][j][k], mismatchIdH[i][j][k], TT, kT);
                pf->expmismatch1nI[i][j][k] =
                    RESCALE_BF(mismatch1nI37[i][j][k], mismatch1nIdH[i][j][k], TT, kT);
                pf->expmismatchH[i][j][k] =
                    RESCALE_BF(mismatchH37[i][j][k], mismatchHdH[i][j][k], TT, kT);
                pf->expmismatch23I[i][j][k] =
                    RESCALE_BF(mismatch23I37[i][j][k], mismatch23IdH[i][j][k], TT, kT);

                if (md->dangles) {
                    pf->expmismatchM[i][j][k] =
                        RESCALE_BF_SMOOTH(mismatchM37[i][j][k], mismatchMdH[i][j][k], TT, kT);
                    pf->expmismatchExt[i][j][k] =
                        RESCALE_BF_SMOOTH(mismatchExt37[i][j][k], mismatchExtdH[i][j][k], TT, kT);
                } else {
    ...
    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j <= NBPAIRS; j++)
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++) {
                    pf->expint11[i][j][k][l] =
                        RESCALE_BF(int11_37[i][j][k][l], int11_dH[i][j][k][l], TT, kT);
                }
        ...
        for (j = 0; j <= NBPAIRS; j++)
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++) {
                    int m;
                    for (m = 0; m < 5; m++) {
                        pf->expint21[i][j][k][l][m] =
                            RESCALE_BF(int21_37[i][j][k][l][m], int21_dH[i][j][k][l][m], TT, kT);
                    }
                }
        ...
        for (j = 0; j <= NBPAIRS; j++)
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++) {
                    ...
                    for (m = 0; m < 5; m++)
                        for (n = 0; n < 5; n++) {
                            pf->expint22[i][j][k][l][m][n] = RESCALE_BF(
                                int22_37[i][j][k][l][m][n], int22_dH[i][j][k][l][m][n], TT, kT);
                        }
                }
    ...
    if (salt_conc == salt_standard) {
        ...
        pf->expSaltStack = exp(-vrna_salt_stack(salt_conc, salt_standard, md->helical_rise) * 10. / kT);
        vrna_salt_ml(pf->SaltLoopDbl, md->saltMLLower, md->saltMLUpper, &pf->SaltMLbase,
                     &pf->SaltMLclosing);
        ...
        for (i = 0; i <= NBPAIRS; i++) pf->expMLintern[i] *= exp(-pf->SaltMLbase * 10. / kT);
...
PRIVATE vrna_exp_param_t* get_exp_params_ali(vrna_md_t* md, unsigned int n_seq, double pfs) {
    ...
    pf->pf_scale = pfs;
    pf->kT = kTn =
        ((double) n_seq) * md->betaScale * (md->temperature + K0) * GASCONST; /* kT in cal/mol  */
    pf_smooth = md->pf_smooth;
    TT = (md->temperature + K0) / (Tmeasure);
    salt_conc = md->salt;
    salt_standard = VRNA_MODEL_DEFAULT_SALT;
    ...
    for (unsigned int i = VRNA_GQUAD_MIN_STACK_SIZE; i <= VRNA_GQUAD_MAX_STACK_SIZE; i++)
        for (unsigned int j = 3 * VRNA_GQUAD_MIN_LINKER_LENGTH;
    ...
    for (unsigned int i = 0; i <= NBPAIRS; i++) /* includes AU penalty */
    ...
    for (unsigned int i = 0; i <= NBPAIRS; i++)
        for (unsigned int j = 0; j <= 4; j++) {
            if (md->dangles) {
                pf->expdangle5[i][j] =
                    RESCALE_BF_SMOOTH(dangle5_37[i][j], dangle5_dH[i][j], TT, kTn);
                pf->expdangle3[i][j] =
                    RESCALE_BF_SMOOTH(dangle3_37[i][j], dangle3_dH[i][j], TT, kTn);
            } else {
                pf->expdangle3[i][j] = pf->expdangle5[i][j] = 1;
            }
        }
    ...
    for (unsigned int i = 0; i <= NBPAIRS; i++)
        for (unsigned int j = 0; j < 5; j++)
            for (unsigned int k = 0; k < 5; k++) {
                pf->expmismatchI[i][j][k] =
                    RESCALE_BF(mismatchI37[i][j][k], mismatchIdH[i][j][k], TT, kTn);
                pf->expmismatch1nI[i][j][k] =
                    RESCALE_BF(mismatch1nI37[i][j][k], mismatch1nIdH[i][j][k], TT, kTn);
                pf->expmismatchH[i][j][k] =
                    RESCALE_BF(mismatchH37[i][j][k], mismatchHdH[i][j][k], TT, kTn);
                pf->expmismatch23I[i][j][k] =
                    RESCALE_BF(mismatch23I37[i][j][k], mismatch23IdH[i][j][k], TT, kTn);

                if (md->dangles) {
                    pf->expmismatchM[i][j][k] =
                        RESCALE_BF_SMOOTH(mismatchM37[i][j][k], mismatchMdH[i][j][k], TT, kTn);
                    pf->expmismatchExt[i][j][k] =
                        RESCALE_BF_SMOOTH(mismatchExt37[i][j][k], mismatchExtdH[i][j][k], TT, kTn);
                } else {
                    pf->expmismatchM[i][j][k] = pf->expmismatchExt[i][j][k] = 1.;
                }
            }
        ...
        for (unsigned int j = 0; j <= NBPAIRS; j++)
            for (unsigned int k = 0; k < 5; k++)
                for (unsigned int l = 0; l < 5; l++) {
                    pf->expint11[i][j][k][l] =
                        RESCALE_BF(int11_37[i][j][k][l], int11_dH[i][j][k][l], TT, kTn);
                }
        ...
        for (unsigned int j = 0; j <= NBPAIRS; j++)
            for (unsigned int k = 0; k < 5; k++)
                for (unsigned int l = 0; l < 5; l++) {
                    ...
                    for (m = 0; m < 5; m++) {
                        pf->expint21[i][j][k][l][m] =
                            RESCALE_BF(int21_37[i][j][k][l][m], int21_dH[i][j][k][l][m], TT, kTn);
                    }
                }
            ...
            for (unsigned int k = 0; k < 5; k++)
                for (unsigned int l = 0; l < 5; l++) {
                    ...
                    for (m = 0; m < 5; m++)
                        for (n = 0; n < 5; n++) {
                            pf->expint22[i][j][k][l][m][n] = RESCALE_BF(
                                int22_37[i][j][k][l][m][n], int22_dH[i][j][k][l][m][n], TT, kTn);
                        }
                }
    ...
    if (salt_conc == salt_standard) {
        ...
        pf->expSaltStack = exp(-vrna_salt_stack(salt_conc, salt_standard, md->helical_rise) * 10. / kTn);

...
#include "ViennaRNA/cephes/mconf.hpp"
...
PRIVATE INLINE double epsilonr(double T) {
    return 5321 / T + 233.76 - 0.9297 * T + 1.417 * T * T / 1000 - 0.8292 * T * T * T / 1000000;
}

PRIVATE INLINE double bjerrum_length(double T) { return 167100.052 / (T * epsilonr(T)); }

PRIVATE INLINE double ionic_strength(double rho) { return rho; }

PRIVATE INLINE double kappa_inv(double rho, double T) {
    return 8.1284 / (sqrt(bjerrum_length(T) * ionic_strength(rho)));
}

PRIVATE INLINE double kappa(double rho, double T) {
    return sqrt(bjerrum_length(T) * ionic_strength(rho)) / 8.1284;
}

PRIVATE INLINE double tau_ds(double T, double Helical_Rise) {
...
PRIVATE INLINE double tau_ss(double T, double backbonelen) {
...
PRIVATE INLINE double pairing_salt_const(double T, double Helical_Rise) {
    return 2 * (GASCONST / 1000.) * T * bjerrum_length(T) * Helical_Rise * tau_ds(T, Helical_Rise) *
           tau_ds(T, Helical_Rise);
}

PRIVATE double approx_hyper(double y) {
    double a, b, c;

    a = 1 / (pow(y, 6.) / pow(2 * PI, 6.) + 1);
    b = pow(y, 4.) / (36 * pow(PI, 4.)) - pow(y, 3.) / (24 * PI * PI) + y * y / (2 * PI * PI) -
        y / 2;
...
PRIVATE double loop_salt_aux(double kmlss, int L, double T, double backbonelen) {
    ...
    a = (GASCONST / 1000.) * T * bjerrum_length(T) * L * backbonelen * tau_ss(T, backbonelen) *
        tau_ss(T, backbonelen);
    b = log(kmlss) - log(PI / 2) + Eular_const + approx_hyper(kmlss) +
        1 / kmlss * (1 - exp(-kmlss) + kmlss * expn(1, kmlss));
...
PUBLIC double vrna_salt_loop(int L, double rho, double T, double backbonelen) {
    ...
    correction =
        loop_salt_aux(kmlss, L, T, backbonelen) - loop_salt_aux(kmlss_ref, L, T, backbonelen);
...
PUBLIC int vrna_salt_stack(double rho, double T, double hrise) {
...
/* LoopEnergy = m*size + b */
PUBLIC void vrna_salt_ml(double saltLoop[], int lower, int upper, int* m, int* b) {
    int sumx, sumxx;
...
PUBLIC int vrna_salt_duplex_init(vrna_md_t* md_p) {
    double x;
    vrna_md_t md;

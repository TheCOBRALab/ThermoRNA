...
static const char* get_log_level_string(vrna_log_levels_e level) {
    switch (level) {
        case VRNA_LOG_LEVEL_DEBUG:    return "[DEBUG]";
        case VRNA_LOG_LEVEL_INFO:     return "[INFO]";
        case VRNA_LOG_LEVEL_WARNING:  return "[WARNING]";
        case VRNA_LOG_LEVEL_ERROR:    return "[ERROR]";
        case VRNA_LOG_LEVEL_CRITICAL: return "[FATAL]";
        default:                      return "[UNKNOWN]";
    }
}
static const char* get_log_level_color(vrna_log_levels_e level) {
    switch (level) {
        case VRNA_LOG_LEVEL_DEBUG:    return ANSI_COLOR_CYAN_B;
        case VRNA_LOG_LEVEL_INFO:     return ANSI_COLOR_BLUE_B;
        case VRNA_LOG_LEVEL_WARNING:  return ANSI_COLOR_YELLOW_B;
        case VRNA_LOG_LEVEL_ERROR:    return ANSI_COLOR_RED_B;
        case VRNA_LOG_LEVEL_CRITICAL: return ANSI_COLOR_MAGENTA_B;
...
void vrna_log(vrna_log_levels_e level, const char* file_name, int line_number,
...
static void log_default(vrna_log_event_t* event) {
    if (!logger.default_file) logger.default_file = stderr;
    ...
    if (isatty(fileno(logger.default_file))) {
        fprintf(logger.default_file, "%s%-9s" ANSI_COLOR_RESET " ",
                get_log_level_color(event->level), get_log_level_string(event->level));
    }

    /* print file name / line number trace unless turned off explicitely */
    if (logger.options & VRNA_LOG_OPTION_TRACE_CALL) {
        if (isatty(fileno(logger.default_file))) {
            fprintf(logger.default_file, "\x1b[90m%s:%d:" ANSI_COLOR_RESET " ", event->file_name,
                    event->line_number);
        }
    }
...
static void log_v(vrna_log_event_t* event) {
    ...
    if (!(logger.options & VRNA_LOG_OPTION_QUIET)) {
        ...
        if (event->level >= logger.default_level) log_default(event);
    }
    ...
    for (size_t i = 0; i < vrna_array_size(logger.callbacks); i++) {
        ...
        if (event->level >= cb->level) cb->cb(event, cb->cb_data);
    }
